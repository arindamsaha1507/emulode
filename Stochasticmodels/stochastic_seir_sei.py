"""The SEIR-SEI compartmental model"""
...
def initialize_parameters(args):
    """Initialize parameters for the SEIR-SEI model."""
    ...
    for paramname in paramnames:
        if not hasattr(args, paramname):
            raise ValueError(f"Parameter {paramname} is missing")
...
def initialize_state(params):
    """Initialize state for the SEIR-SEI model."""
...
def calculate_rates(state, params):
    """Calculate rates for the SEIR-SEI model."""

    number_of_humans = state["S_h"] + state["E_h"] + state["I_h"] + state["R_h"]
    # N_v = state["S_v"] + state["E_v"] + state["I_v"] + state["R_v"]
    rates = {
        "rate_birth_h": params["lambda_h"],
        "rate_death_susceptible_h": params["mu_h"] * state["S_h"],
        "rate_exposed_h": params["beta_h"]
        * state["S_h"]
        * state["I_v"]
        / number_of_humans,
...
def update_state(state, event_type):
    """Update state for the SEIR-SEI model."""
...
def run_simulation(args):
    """Run a simulation of the SEIR-SEI model."""
    ...
    while t < t_max:
        ...
        for i, event_type in enumerate(event_types):
            if i == 0 and event_prob < rate_sums[i] / total_rate:
                event_type = "Birth"
            if (
                i > 1  # pylint: disable=chained-comparison
                and i < len(event_types) - 1
                and event_prob > rate_sums[i - 1] / total_rate
                and event_prob < rate_sums[i] / total_rate
            ):
                event_type = event_type_mapping[event_types[i]]
            if (
                i == len(event_types) - 1  # pylint: disable=chained-comparison
                and event_prob > rate_sums[i - 1] / total_rate
                and event_prob < rate_sums[i] / total_rate
            ):
...
def plot_results(events):
    """Plot the results of the SEIR-SEI model simulation."""
    ...
    ax1.plot(
        times, s_values, label="Susceptible (Human)", linestyle="--", color="tab:blue"
    )
    ax1.plot(
        times, e_values, label="Exposed (Human)", linestyle="-", color="tab:orange"
    )
    ax1.plot(
        times, i_values, label="Infectious (Human)", linestyle="-", color="tab:green"
    )
    ax1.plot(
        times,
        i_h_d_values,
        label="Disease deaths (Human)",
        linestyle="-",
        color="tab:red",
    )
    ax1.set_xlabel("Time")
    ...
    ax2.plot(
        times,
        s_v_values,
        label="Susceptible (Vector)",
        linestyle="--",
        color="tab:cyan",
    )
    ax2.plot(
        times, e_v_values, label="Exposed (Vector)", linestyle="-", color="tab:purple"
    )
    ax2.plot(
        times, i_v_values, label="Infectious (Vector)", linestyle="-", color="tab:pink"
    )
    ...
    plt.grid(True)
    plt.savefig("StochasticSEIR_SEI.png")
    plt.show()


def main():
    """Run the SEIR-SEI model simulation and plot the results."""

    # tmax = 10
    parser = argparse.ArgumentParser()
    defaults_vals = {
        "--tmax": 50,  # Maximum time for simulation
    ...
    for key, value in defaults_vals.items():
        parser.add_argument(
            key, type=type(value), default=value, help=defaults_help[key]
        )

    args = parser.parse_args()
    events = run_simulation(args)
    save_path = "./"
    with open(save_path + "SEIR_SEI_events.csv", "w", encoding="utf-8") as f:
        for event in events:
            f.write(str(event) + "\n")
